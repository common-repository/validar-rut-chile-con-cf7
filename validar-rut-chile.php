	...
	$plugin_messages = '';
	 ...
	 if ( file_exists( WP_PLUGIN_DIR . '/contact-form-7/wp-contact-form-7.php' ) ){
		if(!is_plugin_active( 'contact-form-7/wp-contact-form-7.php' ))	{
			...
			$plugin_messages = __( 'The <strong>Validar RUT Chile con CF7</strong> plugin requires the Contact Form 7 plugin to be activated.', 'validar-rut-chile-con-cf7' );
		}
	 }else{
		// Download Contact form 7
		$plugin_messages = __( 'The <strong>Validar RUT Chile con CF7</strong> plugin depends on the Contact Form 7 plugin to work. <a href="https://wordpress.org/plugins/contact-form-7/" target="_blank">Download it here</a>.', 'validar-rut-chile-con-cf7' );
	 }
			...
			if($name == 'rut') {
				...
				if($cf7rut != '') {
					if(cf7rut_validation_callback_action($cf7rut) == false){
						$result->invalidate( $tag, __( 'Invalid RUT.', 'validar-rut-chile-con-cf7' ));					
					}else{
